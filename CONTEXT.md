...
### 2026-02-07 (오후 - 아홉 번째 수정) 🧴 카테고리 필터링 강화
- **화장품/생활용품 오매칭 방지**
  - **문제**: "베이킹 파우더" 검색 시 클렌징폼(화장품) 검색됨
  - **해결**: `calculateMatchScore()` 함수에 비식품 키워드 필터링 추가 (-100점)
    - 화장품: 클렌징, 로션, 크림, 에센스, 세럼, 토너 등
    - 생활용품: 세제, 샴푸, 린스, 비누, 치약 등
    - 화장품 브랜드: 에뛰드, 이니스프리, 토니모리 등
  - **결과**: 식재료 검색 시 화장품/생활용품 완전히 제외
  - **수정된 파일**: `packages/scraper/src/index.ts`

### 2026-02-07 (오후 - 열 번째 수정) 🎯 옵션 선택 로직 최적화
- **옵션 선택 감지 순서 개선**
  - **문제**: 옵션 선택 로직이 작동하지 않음 (로그에 아무것도 안 나옴)
  - **해결**: 
    1. 순서 변경: 셀렉트박스 → 라디오 버튼 → div[role="button"] → 일반 버튼
    2. 모든 요소 개수 확인 후 로그 출력
    3. 보이는 요소만 클릭 (visibility 체크)
    4. 대기 시간 1.5초로 증가 (UI 업데이트 보장)
  - **상세 로그 추가**: 각 단계별 Found X elements 로그로 디버깅 용이
  - **수정된 파일**: `packages/scraper/src/index.ts`
